#!/usr/bin/env python3
"""
ikizamini_local_parallel_cli.py

Parallel/concurrent CLI runner for IKIZAMINI using Ollama (e.g. gemma3).
...
Examples:
  python3 ikizamini_local_parallel_cli.py --input Uru.txt
  python3 ikizamini_local_parallel_cli.py --input Uru.txt --workers 4
  python3 ikizamini_local_parallel_cli.py --input Uru.txt --worker-model gemma3 --manager-model gemma3
...
How to run:
    ...
    python3 ikizamini_local_parallel_cli.py --input Uru.txt --worker-model gemma3 --manager-model gemma3 --workers 4

    # Limit objectives + custom output folder
    python3 ikizamini_local_parallel_cli.py --input Uru.txt --limit 10 --output-dir Parallely_Processed --workers 4

    # Custom Ollama URL / context / timeout
    python3 ikizamini_local_parallel_cli.py --input Uru.txt --ollama-url http://localhost:11434 --num-ctx 8192 --timeout-s 600 --workers 4
"""
...
def ollama_smoke_check(base_url: str, model: str, timeout_s: int = 20) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Smoke check failed: model did not return JSON. Content preview: {content[:200]}"
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--input", help="Path to objectives outline .txt (e.g. Uru.txt). Required unless --smoke-check."
    )
    ap.add_argument(
        "--output-dir",
    ...
    ap.add_argument(
        "--timeout-s",
        type=int,
        default=int(os.environ.get("IKIZAMINI_OLLAMA_TIMEOUT", "600")),
        help="Ollama request timeout seconds",
    )
    ap.add_argument(
        "--max-retries",
        type=int,
        default=int(os.environ.get("IKIZAMINI_OLLAMA_RETRIES", "4")),
        help="Retries per Ollama call",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=60,
        help="Seconds to wait for Ollama to become ready (default: 60)",
    )
    ap.add_argument(
        "--workers",
        type=int,
    ...
    objectives: List[Objective] = [
        Objective(o["objective_id"], o["objective_text"], o["path"]) for o in objs_raw
    ]
    ...
    print(
        f"[START] Objectives: {total} | concurrency={args.workers} | model={args.worker_model}/{args.manager_model}"
    )
